      ...
      <div className="container-luxury">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
          {events.map((e, idx) => (
            <motion.div 
              key={idx}
              className="glass-card luxury-card p-10 text-center"
              initial={{ opacity: 0, y: 30 }}
              whileInView={{ opacity: 1, y: 0 }}
              viewport={{ once: true }}
              transition={{ delay: idx * 0.1 }}
            >
              <h3 className="font-playfair text-2xl text-gold mb-2">{e.title}</h3>
              <p className="font-montserrat text-sm tracking-[2px] text-ivory/80 uppercase mb-4">{e.date} • {e.time}</p>
              <div className="w-12 h-[1px] bg-gold/30 mx-auto mb-4" />
              <p className="font-lora text-ivory/70">{e.desc}</p>
            </motion.div>
          ))}
        </div>
      </div>
